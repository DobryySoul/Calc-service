...
Перейдя в браузере по адресу http://localhost:9090/api/v1/register (если запускали на дефолтном значении переменных окружения), вы попадете на внешний интерфейс сервиса

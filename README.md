...
- `/api/v1/calculate` - отправить новое выражение для вычисления[^1].
- `/api/v1/expressions` - получить список всех выражений[^2].
- `/api/v1/expression/:id` - получить выражение по идентификатору id[^3].
- `/internal/task` - получить задачу для обработки/отправить результат[^4].

    - GET: отдает задачу на выполнение[^5].

    - POST: отправляет результат выполнения[^6]. 
    ```json
    {
...
[^1]:- `201`: Ответ на добавление выражения для вычисления в верном формате:
...
[^2]:- `200`: Успешно получен список выражений:
